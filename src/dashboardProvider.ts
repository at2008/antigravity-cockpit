...
export class DashboardProvider {
    ...
    private async _update() {
        ...
        this._panel.webview.html = this._getHtmlForWebview(accountsDataPhase1, groupsConfig);
    ...
    private _getHtmlForWebview(accountsData: any[], groupsConfig: any) {
        ...
        return `<!DOCTYPE html>
            ...
            <head>
                ...
                <style>
                    ...
                    .modal-body {
                        padding: 8px 8px 30px 8px; /* Increased bottom padding for dropdowns */
                        overflow-y: auto;
                        flex: 1;
                    }
                    .modal-footer {
                    ...
                    .model-dropdown-content {
                        display: none;
                        position: absolute;
                        top: 100%; /* Change to open downward */
                        left: 0;
                        background: var(--vscode-sideBar-background);
                        border: 1px solid var(--border-color);
                        border-radius: 8px;
                        ...
                        margin-top: 4px; /* Space from button */
                    }
                    ...
                    <table class="account-table">
                        <thead>
                            <tr>
                                ...
                                <th>${t('accountEmailHeader')}</th>
                                <th>${t('accountNameHeader')}</th>
                                <th>${t('accountTierHeader')}</th>
                                <th>${t('groupQuotaHeader')}</th>
                ...
                <script>
                    ...
                    function renderListView() {
                        ...
                        accounts.forEach(acc => {
                            ...
                            // 计算各分组配额显示
                            let groupQuotasHtml = '';
                            let groupTitles = []; // 移除类型标注，Webview 脚本必须是纯 JS
                            ...
                            if (!acc.quota || acc.quota.is_error) {
                                ...
                                groupTitles.push(acc.quota?.is_error ? msg('quotaFetchError') : msg('quotaPendingRefresh'));
                            } else {
                                groupQuotasHtml = groupsConfig.groups.map(group => {
                                    ...
                                    const quotaText = found ? \`\${minPercentage}%\` : msg('noAccount');
                                    groupTitles.push(\`\${group.name}: \${quotaText}\`);

                                    if (!found) {
                                        return \`<div class="group-quota-bar no-data"></div>\`;
                                    }

                                    let color = '#4ade80'; // 绿色
                                    if (minPercentage <= 20) color = '#f87171'; // 红色
                                    else if (minPercentage <= 50) color = '#fbbf24'; // 黄色
                            ...
                            const stackFullTitle = groupTitles.join('\\n');
                            ...
                            tr.innerHTML = \`
                                ...
                                <td><div class="group-quota-stack" title="\${stackFullTitle}">\${groupQuotasHtml || '-'}</div></td>
                                <td style="color:var(--text-secondary);font-size:12px;">\${lastUsedStr}</td>
                                <td style="text-align: right;">
                                    <div class="btn-group" style="justify-content: flex-end;">
                                        \${!acc.isCurrent ? \`<button class="teal" onclick="switchAccount('\${acc.id}', '\${acc.email}')">\${msg('switchBtn')}</button>\` : \`<span style="font-size:12px;color:#4ade80;margin-right:10px;">\${msg('currentAccountLabel')}</span>\`}
                                        <button class="secondary" onclick="refreshAccount('\${acc.id}')">\${msg('refreshBtn')}</button>
                                        <button class="secondary" onclick="exportToken('\${acc.id}')">🔑</button>
...
function render() {
    ...
    accounts.forEach(acc => {
        ...
        tab.innerHTML = \`<span>\${shortEmail}</span>\${acc.isCurrent ? \`<span class="badge">\${msg('currentAccountLabel').toUpperCase()}</span>\` : ''}\`;
                            tab.onclick = () => {
