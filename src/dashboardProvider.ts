...
export class DashboardProvider {
    ...
    private async _update() {
        ...
        // === 第二阶段：后台获取缺失的配额数据 ===
        const needsFetch = accountsDataPhase1.some(acc => acc.token && !acc.quota);
        if (needsFetch) {
                ...
                <script>
                    ...
                    function renderListView() {
                        ...
                        if (accounts.length === 0) {
                            tbody.innerHTML = \`<tr><td colspan="6" style="text-align:center;padding:30px;">\${msg('noAccount')}</td></tr>\`;
                            return;
                        }
